...
有明確抵達/會議/預約時間的 stop flexible=false；只有「想去、順路、途中」而沒有固定時間的 stop flexible=true。不可捏造地點；不確定時保留使用者原文地點。`;
...
export async function extractIntent(apiKey: string, messages: Array<{ role: "user" | "assistant"; content: string }>): Promise<ItineraryIntent> {
  const model = new GoogleGenerativeAI(apiKey).getGenerativeModel({ model: "gemini-2.5-flash", systemInstruction: instruction });
